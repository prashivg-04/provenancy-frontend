...
import {
  ArrowLeft, ShieldCheck, FileCheck, Lock,
  Network, Hexagon, Terminal, BadgeCheck, Activity, Loader2, Building
} from 'lucide-react'
import { getSupervisorPublic } from '../lib/api'
...
function NotFound() {
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-6">
      ...
      <h2 className="text-xl font-light text-foreground mb-2 tracking-tight">Oracle Profile Not Found</h2>
      <p className="text-sm text-muted-foreground mb-8 max-w-sm leading-relaxed">
        This supervisor profile does not exist or has been deactivated.
      </p>
      <Link
        to="/"
...
export default function PublicSupervisorProfile() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await getSupervisorPublic(id)
        setData(res.data)
      } catch (err) {
  ...
  const isInstitutional = data?.trust_tier === 'institutional'
      ...
      <main className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12 py-16 pb-32">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Fetching Oracle Record...</span>
          </div>
        )}
            ...
            <header className="mb-16 relative">
              ...
              <div className="bg-card/30 backdrop-blur-2xl border border-border/20 rounded-[2.5rem] p-10 lg:p-16 flex flex-col lg:flex-row gap-10 lg:items-center justify-between shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] overflow-hidden relative">
                ...
                <div className="flex items-center gap-10 relative z-10">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                       ...
                       <span className="text-[11px] font-bold uppercase tracking-[0.3em] text-primary bg-primary/10 px-3 py-1 rounded-full border border-primary/20">
                         {isInstitutional ? 'Institutional Oracle' : 'Independent Oracle'}
                       </span>
                    </div>
                    <div>
                      ...
                      <p className="text-xl lg:text-2xl text-muted-foreground font-light mt-2 tracking-wide">
                        {[data.designation, data.organization].filter(Boolean).join(' · ')}
                      </p>
                    </div>
                    {data.bio && (
                ...
                <div className="flex flex-col gap-4 text-left lg:text-right bg-background/60 p-6 rounded-2xl border border-border/20 backdrop-blur-md relative z-10 min-w-[300px]">
                  ...
                  <div>
                    <span className="block text-[10px] text-muted-foreground uppercase tracking-[0.2em] font-bold mb-1">Total Verifications</span>
                    <span className="text-sm font-mono text-foreground">{engagements.length}</span>
                  </div>
                  <div>
                    <span className="block text-[10px] text-muted-foreground uppercase tracking-[0.2em] font-bold mb-1">Trust Tier</span>
                    <span className={`text-xs font-bold uppercase tracking-widest ${isInstitutional ? 'text-primary' : 'text-muted-foreground'}`}>
                      {data.trust_tier ?? '—'}
                    </span>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 w-full items-start">
              ...
              <div className="lg:col-span-6 space-y-8 relative">
                ...
                <div className="flex items-center justify-between mb-8 overflow-hidden rounded-2xl border border-border/20 bg-background/50 backdrop-blur-md p-4 px-6 md:px-8 shadow-sm">
                   ...
                   <span className="px-3 py-1 bg-border/30 rounded text-[10px] font-mono tracking-widest text-muted-foreground uppercase">{engagements.length} Signatures</span>
                </div>
                          ...
                          <div className="flex flex-col gap-6 relative z-10">
                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                              <div className="space-y-1 max-w-[80%]">
                                ...
                                <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">{eng.organization_name}</p>
                              </div>
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 pt-6 border-t border-border/15">
                              <div className="space-y-1.5">
                                ...
                                <span className="text-sm font-medium">{eng.student_full_name ?? '—'}</span>
                              </div>
                              <div className="space-y-1.5">
                                <span className="block text-[9px] uppercase tracking-[0.2em] font-bold text-muted-foreground">Certified Role</span>
                                <span className="text-sm text-foreground">{eng.role}</span>
                              </div>
                              <div className="space-y-1.5 col-span-2 lg:col-span-1">
                                <span className="block text-[9px] uppercase tracking-[0.2em] font-bold text-muted-foreground">Ledger Timestamp</span>
                                <span className="text-[11px] font-mono p-1.5 bg-background border border-border/30 rounded text-muted-foreground/80 tracking-widest">
                                  {fmtDate(eng.verified_at)}
                                </span>
                              </div>
                            </div>
                          </div>
